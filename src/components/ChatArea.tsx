...
export default function ChatArea() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const filePath = `chat_media/${chatId}/${fileId}.${fileExtension}`;
  ...
  return (
    <div className="flex-1 flex flex-col h-full">
      {/* Header */}
      <div className="h-12 border-b border-zinc-800 flex items-center justify-between px-4 shadow-sm bg-zinc-800 z-10">
      ...
      <div className="p-4 bg-zinc-800">
        <form onSubmit={handleSendMessage} className="bg-zinc-700 rounded-lg flex items-center px-4 py-2 gap-2">
          <input 
            type="file" 
            ref={fileInputRef} 
          ...
          <button 
            ...
            disabled={uploading}
            className="text-zinc-400 hover:text-zinc-200 p-2 rounded-full hover:bg-zinc-600 transition-colors disabled:opacity-50"
          >
            {uploading ? <div className="w-5 h-5 border-2 border-zinc-400 border-t-transparent rounded-full animate-spin" /> : <Paperclip size={20} />}
          </button>
          <input
            type="text"
            value={newMessage}
